...
if __name__ == '__main__':
    ...
    parser.add_argument('--detect_model', type=str, default='weights/plate_detect.pt', help='model.pt path')
    parser.add_argument('--image_path', type=str, default='imgs', help='source folder')
    parser.add_argument('--img_size', type=int, default=640, help='inference size (pixels)')
    parser.add_argument('--output', type=str, default='result1', help='output folder')
    ...
    if not os.path.isfile(opt.image_path):  # 如果是目录
        file_list = [os.path.join(opt.image_path, f) for f in os.listdir(opt.image_path) if os.path.isfile(os.path.join(opt.image_path, f))]
        for img_path in file_list:
            ...
            result = draw_result(img,dict_list)
            print("车牌信息：",result)
    ...
    avg_time = time_all / (len(file_list) - 1) if len(file_list) > 1 else total_time
    print(f"Total time: {total_time:.2f} s, Average time per image: {avg_time:.2f} s")
# if __name__ == '__main__':
#     parser = argparse.ArgumentParser()
