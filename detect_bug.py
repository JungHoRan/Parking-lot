...
def generate_sequential_filenames(folder_path, extension='jpg'):
    """
    Generate a list of sequential filenames based on the number of files in the folder.

    Parameters:
    folder_path (str): The path to the folder containing the files.
    extension (str): The extension to use for the filenames (default is 'jpg').

    Returns:
    list: A list of sequential filenames.
    """
    files = [f for f in os.listdir(folder_path) if os.path.isfile(os.path.join(folder_path, f))]
    num_files = len(files)
    return [os.path.join(folder_path, f"{i}.{extension}") for i in range(1, num_files + 1)]
...
if __name__ == '__main__':
    ...
    parser.add_argument('--image_path', type=str, default=r'D:\data\tiny\weather', help='source')  # 图片路径
    ...
    # 生成顺序文件名列表
    sequential_filenames = generate_sequential_filenames(opt.image_path)
    print("总共有几张：", len(sequential_filenames))
    print("路径：", sequential_filenames)
    for img_path in sequential_filenames:
        time_b = time.time()
